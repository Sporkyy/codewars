...
const longestSlideDown = pyramid => {
  ...
  const recurseOfTheMummy = block => {
    ...
    const ls = recurseOfTheMummy(left);
    const rs = recurseOfTheMummy(right);
    const result = Math.max(Math.max(ls, rs) + val, val);
    memo.set(block, result);
    return result;
  };
  return recurseOfTheMummy(p.pyramidion);
};
