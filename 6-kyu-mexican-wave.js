  ...
  for (let i = 0; i < str.length; i++)
    if (/\w/.test(str[i]))
      arr.push(
        str.replace(
          new RegExp(`(.{${i}})(.)(.*)`),
          (_, p1, p2, p3) => `${p1}${p2.toUpperCase()}${p3}`,
        ),
      );
  return arr;
};
...
deepStrictEqual(wave('hello'), ['Hello', 'hEllo', 'heLlo', 'helLo', 'hellO']);

deepStrictEqual(wave('codewars'), [
  'Codewars',
  'cOdewars',
  'coDewars',
  'codEwars',
...
deepStrictEqual(wave(''), []);

deepStrictEqual(wave('two words'), [
  'Two words',
  'tWo words',
  'twO words',
  'two Words',
...
deepStrictEqual(wave(' gap '), [' Gap ', ' gAp ', ' gaP ']);
