...
all cells of the grid with digits from 1 to 9, so that each column, each
row, and each of the nine 3x3 sub-grids (also known as blocks) contain
...
a valid solution, or false otherwise. The cells of the sudoku board may
also contain 0's, which will represent empty cells. Boards containing one
...
const validSolution = board => {
  ...
  for (let i = 0; i < 9; i++) {
    for (let j = 0; j < 9; j++) {
      ...
      cnts[board[j][i]]++;
      cnts[board[j % 3][i % 3]]++;
      if (0 < cnts[0]) return false;
    }
  }
  return cnts.slice(1).every(n => 27 === n);
};
