...
while you wait for the answer. But if you try it here in Code Wars you will most
...
//   return memo.set(n, fibonacci(n - 1) + fibonacci(n - 2)).get(n);
// };
...
//       : memo.has(n)
//       ? memo.get(n)
//       : memo.set(n, rec(n - 1) + rec(n - 2)).get(n);
//   return rec(n);
// };
...
const fibonacci = n => {
  const m = new Map([[1, 1]]);
  const r = n => (!n ? 0 : m.get(n) || m.set(n, r(n - 1) + r(n - 2)).get(n));
  return r(n);
};
...
strictEqual(fibonacci(70), 190392490709135);
strictEqual(fibonacci(60), 1548008755920);
strictEqual(fibonacci(50), 12586269025);
