...
If you finish this kata, you can try Insane Coloured Triangles by Bubbler, which
is a much harder version of this one.
...
//     // console.log(obj[`${row[i]}${row[i + 1]}`]);
//     res += obj[`${row[i]}${row[i + 1]}`];
//   }
//   return 1 === res.length ? res : triangle(res);
// };
...
strictEqual(triangle('GB'), 'R');
strictEqual(triangle('RRR'), 'R');
strictEqual(triangle('RGBG'), 'B');
strictEqual(triangle('RBRGBRB'), 'G');
strictEqual(triangle('RBRGBRBGGRRRBGBBBGG'), 'G');
